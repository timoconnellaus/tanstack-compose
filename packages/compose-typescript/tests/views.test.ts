...
const write = (view: string) => ({
  toolCalls: [
    { name: 'write_plugin', args: { id: 'summariser', source: plugin, view } },
  ],
})
...
describe('a view checked against the plugin it belongs to', () => {
  ...
  it('makes a view calling a handler the plugin does not export a diagnostic', async () => {
    ...
    expect(listed?.viewDeclarations).toContain(
      'type GrantedSlot = "chat.input.actions"',
    )
    expect(listed?.viewDeclarations).toContain('declare const slots')
    ...
    expect(handler?.viewDeclarations).toContain(
      'summarise: (input: { text: string; }) => string',
    )
    expect(listAfterWrite[0]).not.toContain('summariser')
    ...
    expect(written?.entries.map((one) => one.id)).toEqual([
      'summariser',
      'summariser.view',
    ])
