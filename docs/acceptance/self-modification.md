...
- **E1** One test runs a conversation against the scripted provider in which the model: lists the plugins, disables one that a tool depends on, sees the dependent go `pending`, re-enables it, adds a plugin from the catalog, uses a tool that plugin provides in the next step, writes a plugin as source that throws on setup, reads the error, rewrites it, uses the tool it registers in the next step, attempts to disable a protected entry and is refused, and removes the plugin it wrote; the test asserts the session log, the final plugin list, and that the client holds no leaked resources.
