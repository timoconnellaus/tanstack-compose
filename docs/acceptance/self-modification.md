...
- **D3** Rewriting an entry's source runs the instance's cleanups and restarts it with the new code; nothing the previous code registered or held survives the rewrite. A rewrite is refused unless the agent has read the entry's current source in this session, and refused with a result saying to re-read if the source has changed since that read.
- **D4** Source that fails to type-check (D7), fails to parse, fails to load, throws during setup, or throws on the first call into it leaves that entry in `error` with the message and, where available, the line; the tool result carries the error in the same shape for every one of these, so the model corrects and rewrites in the same turn with one recovery loop; no other instance is affected.
