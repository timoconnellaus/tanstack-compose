...
The log is not a kernel concern. The kernel reconciles one list; the log lives where the list persists, in the tenant's Durable Object in `@tanstack/start-compose`, as a pure module `@tanstack/compose/generations` (reducer + types, no I/O, outside the kernel's size budget) that any persistence layer can use.
